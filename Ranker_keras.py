'''
TO START THIS SCRIPT YOU NEED FOLLOWING LIBRARIES:
...
class Ranker_NN():
        ...
        def convolutional_neural_network(self):
            ...
            '''
        def make_prediction(self,raw_image_source):
        ...
        def train_model(self, data, true_labels, batch_size, no_epochs):
            self.model_final.compile(optimizer = 'adam', loss = 'categorical_crossentropy',metrics=['accuracy'])
...
'''
create_dummy_labels is a second dummy function, which allows to test this code
independently of the Piotr's image loader.
It takes some integer and returns a numpy array of size (no_of_elements,2),
where two elements in every row simulate the labels (either 1 or 0) for each training
sample.
'''  
...
'''
create_training_data is a dummy function, which allows to test this code
independently of the Piotr's image loader.
It takes images from the string 'raw_image_source', which is the path and
