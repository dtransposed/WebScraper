...
class Ranker_NN():
        '''
        Ranker takes three arguments: n_classes (=1), number of fully connected nodes for custom layer 1 and 2  
        ...
        def __init__(self,n_classes,n_fully_connected_1,n_fully_connected_2):
            ...
            '''
            '''
            def make_prediction(self,np_array_from_Piotr):
                return self.model_final.prediction(np_array_from_Piotr)
...
def create_dummy_labels(no_of_elements):
    dummy_list=np.array([[0]])
    possible_labels=[0,1]
    for number in range(no_of_elements):
        row=np.array([[random.choice(possible_labels)]])
        print(row)
